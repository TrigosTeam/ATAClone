...
```{r}
leiden_clusters <- iterative_cluster_sim(stable_counts_filtered, nb_overdispersion, npcs, discard_pcs, knn_k, tolerance = 0.05, start = 0)
...
set.seed(100)
gex_clusters <- igraph::cluster_leiden(gex_knn_graph, resolution = (knn_k - 1) / (gorder(gex_knn_graph) - 1))
...
doublet_thresholds <- c(10 ** 3.7, 10 ** 3.7, 10 ** 3.85, 10 ** 3.85, 10 ** 3.7, 10 ** 3.85)
```
...
#### Code
```{r}
cn_fit_out <- ATAClone:::fit_cn_clusters(stable_counts_filtered, leiden_clusters, 3, F, cluster_outliers | is_doublet, 1000, 0.7, 2.3, 0.1, T, T)
absolute_cn_list <- get_absolute_copy_number(stable_counts_filtered, leiden_clusters, 3, cn_fit_out$scale_factors, cluster_outliers, is_doublet, F)
