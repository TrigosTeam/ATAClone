...
```{r}
#we COULD estimate this from the data - but might not be as straight-forward as it appears...
...
lib_size <- estimate_lib_size_pca(stable_counts_filtered)

#fit a negative binomial regression to bin counts accounting for technical factors
nb_glm <- glmGamPoi::glm_gp(data = stable_counts_filtered, design = ~ log(betas), size_factors = lib_size, overdispersion = nb_overdispersion)

#simulate counts based on glm fit - use to check mean-variance relationship
stable_counts_sim <- simulate_counts2(nb_glm$Mu, nb_overdispersion)
stable_counts_sim_norm <- exp(log((stable_counts_sim + pseudo_count) ** lambda) - log((nb_glm$Mu + pseudo_count) ** lambda) + log(rowMeans((stable_counts_sim + pseudo_count) ** lambda)))
loess_df <- data.frame(sim.mean = rowMeans(stable_counts_sim_norm), sim.var = rowVars(stable_counts_sim_norm))
loess_mean_var <- loess(sim.var ~ sim.mean, loess_df, span = 0.2)

#normalise counts and check against simulated mean-variance relationship
stable_counts_filtered_norm <- as.matrix(exp(log((stable_counts_filtered + pseudo_count) ** lambda) - log((nb_glm$Mu + pseudo_count) ** lambda) + log(rowMeans((stable_counts_filtered + pseudo_count) ** lambda))))

norm_df <- data.frame(mean = rowMeans(stable_counts_filtered_norm), var = rowVars(stable_counts_filtered_norm))
ggplot(norm_df, aes(x = mean, y = var)) + geom_point() + stat_function(fun = function(x){predict(loess_mean_var, x)})
```

### PCA
We have already assumed a set of principal components represent unwanted technical variation in the previous section. Here we verify this selection by comparing to known scATAC-seq technical factors. **UPDATE** - ATAClone now uses a GLM to model and remove technical factors instead of PCA - but these are still useful visualisations to confirm unwanted factors
...
```{r}
#this is PCA on transformed counts (not the counts corrected after glm fitting) so will still contain unwanted factors
stable_counts_filtered_norm_pca <- get_pca(normalise_counts(stable_counts_filtered, nb_overdispersion, pseudo_count, lambda), npcs)
...
pca_df2 <- data.frame(stable_counts_filtered_norm_pca$x, sqrt_stable_total = sqrt(atac_stable_total_filtered))
...
pca_df2 <- data.frame(stable_counts_filtered_norm_pca$x, beta = betas)
...
```{r}
leiden_clusters <- iterative_cluster_sim_glm(x = stable_counts_filtered, overdispersion = nb_overdispersion, regress.var = betas, pseudo_count = pseudo_count, lambda = lambda, npcs = npcs, k = knn_k, seed = use_seed, iter.limit = 2, tolerance = 0.05)
#also set seed to keep UMAP constant
set.seed(use_seed)
#this is PCA on the counts normalised and corrected after GLM fitting - should be corrected for unwanted factors
stable_counts_filtered_norm_pca2 <- get_pca(stable_counts_filtered_norm, npcs)
umap_embeddings <- get_umap(stable_counts_filtered_norm_pca2$x, npcs, c())
cluster_outliers <- get_cluster_outliers(stable_counts_filtered_norm_pca2$x, leiden_clusters, npcs, c(), knn_k, 0.2)

#also score for cell cycle - mid/late S phase can correspond to its own cluster due to replication timing
gene.rankings <- AUCell::AUCell_buildRankings(as.matrix(gex_counts_filtered), plotStats = F, verbose = F)
cycling.score <- AUCell::AUCell_calcAUC(Seurat::cc.genes.updated.2019, gene.rankings)
s_score <- cycling.score@assays@data$AUC[1,]
g2m_score <- cycling.score@assays@data$AUC[2,]
...
ggplot(cluster_df, aes(x = UMAP_1, y= UMAP_2, color = s_score)) + geom_point() + scale_color_gradient(low = "blue", high = "yellow")
ggplot(cluster_df, aes(x = UMAP_1, y= UMAP_2, color = g2m_score)) + geom_point() + scale_color_gradient(low = "blue", high = "yellow")
```
...
stable_counts_ref <- stable_counts_ref[,colSums(stable_counts_ref) > 1000]
plot_copy_number(stable_counts_filtered, stable_counts_ref, leiden_clusters, stable_counts_filtered_norm_pca2, c(), is_ref_female = F)
```
...
```{r}
set.seed(use_seed2)
#settings per section 1.5.5 here: https://www.bioconductor.org/packages//release/bioc/vignettes/scDblFinder/inst/doc/scDblFinder.html
discard_pcs <- 1
scdbl_out <- scDblFinder::scDblFinder(stable_counts_filtered, dims = npcs,
                                      processing = scDblFinder_custom_function, dbr = 0.03 * ncol(stable_counts_filtered) / 1000, nrounds = 2, removeUnidentifiable = F)
...
```{r}
#override automatic threshold if you wish
override_idx <- c()
override_val <- c()
```
...
doublet_thresholds[override_idx] <- override_val
...
for (i in levels(leiden_clusters)){
  ...
  plot(ggplot(doublet_df, aes(x = atac.peak.count, y = rna.intron.count, color = is.dbl)) + geom_point() + scale_x_log10() + scale_y_log10() + geom_vline(xintercept = doublet_thresholds[as.integer(i)]) + ggtitle(paste0("Cluster ", i)))
}
```
...
absolute_cn_list_smoothed <- get_absolute_copy_number2(stable_counts_filtered, leiden_clusters, ref_cluster, cn_fit_out$scale_factors, is_excluded, F)
...
plot_absolute_cn2(absolute_cn_list_smoothed, y_lim = y_lim, exclude_clusters = c("all",exclude_clusters))
```
