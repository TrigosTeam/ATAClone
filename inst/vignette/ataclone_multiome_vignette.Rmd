...
lambda <- find_lambda(stable_counts_filtered, nb_overdispersion, pseudo_count)
...
gex_knn_graph <- scran::buildKNNGraph(t(gex_counts_filtered_norm_pca$x[,3:25]), k = knn_k)
set.seed(100)
gex_clusters <- igraph::cluster_leiden(gex_knn_graph, resolution = (knn_k - 1) / (igraph::gorder(gex_knn_graph) - 1))
ggplot(data.frame(gex_umap_embeddings, cluster = factor(gex_clusters$membership)), aes(x = UMAP_1, y = UMAP_2, color = cluster)) + geom_point()
scdlb_out <- scDblFinder::scDblFinder(gex_counts_filtered, gex_clusters$membership, dbr = 0.02 * ncol(gex_counts_filtered) / 1000)
...
doublet_thresholds <- c(10 ** 3.75, 10 ** 3.75, 10 ** 3.85, 10 ** 3.85, 10 ** 3.85, 10 ** 3.85, 10 ** 3.75)
```
...
for (i in levels(leiden_clusters)){
  ...
  doublet_df <- data.frame(atac.peak.count = colSums(stable_counts_filtered[,leiden_clusters == i & !cluster_outliers]), 
                           ...
                           doublet.score = scdlb_out$scDblFinder.score[leiden_clusters == i & !cluster_outliers]
  )
  plot(ggplot(doublet_df, aes(x = atac.peak.count, y = rna.intron.count, color = doublet.score)) + geom_point() + scale_x_log10() + scale_y_log10() + geom_vline(xintercept = doublet_thresholds[as.integer(i)]) + scale_color_gradient(low = "blue", high = "yellow"))
}
```
...
cn_fit_out <- ATAClone:::fit_cn_clusters(stable_counts_filtered, leiden_clusters, 5, F, is_excluded, 1000, 0.7, 2.3, 0.1, T, T)
