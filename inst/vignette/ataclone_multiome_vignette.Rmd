...
Computing bin counts from stably-accessible regions is generally sufficient to remove most transposition efficiency bias. However, droplets with a low transposition efficiency (low stable fraction) sometimes look artificially different and can be removed.
...
```{r}
ggplot(qc_df, aes(x = stable_frac, y = atac_stable_total, color = non_zero_residual > min_non_zero_residual)) + geom_point() + scale_y_log10() + geom_vline(xintercept = min_stable_frac)
ggplot(qc_df, aes(x = stable_frac, y = atac_all_total, color = non_zero_residual > min_non_zero_residual)) + geom_point() + 
...
doublet_thresholds <- c(10 ** 3.65, 10 ** 3.65, 10 ** 3.8, 10 ** 3.65)
```
