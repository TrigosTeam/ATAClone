...
```{r}
leiden_clusters <- iterative_cluster_sim(stable_counts_filtered, nb_overdispersion, npcs, discard_pcs, knn_k, tolerance = 0.1)
...
doublet_thresholds <- c(10 ** 3.7, 10 ** 3.7, 10 ** 3.85, 10 ** 3.85, 10 ** 3.75)
```
...
#### Code
```{r}
cn_fit_out <- ATAClone:::fit_cn_clusters(stable_counts_filtered, leiden_clusters, 4, F, cluster_outliers | is_doublet, 1000, 0.7, 2.25, 0.05, T, T)
absolute_cn_list <- get_absolute_copy_number(stable_counts_filtered, leiden_clusters, 4, cn_fit_out$scale_factors, cluster_outliers, is_doublet, F)
